...
describe('shared simulation clock', () => {
  it('scrubs safely, stays paused through visibility, and resumes with a fresh timestamp', () => {
    const clock = start()
    tick(0)
    tick(1000)
    clock.setSpeed(365)
    clock.scrub(1234)
    expect(clock.simulatedDays()).toBe(1234)
    expect(clock.speed()).toBe(365)
    ...
    expect(clock.simulatedDays()).toBe(1234)
    tick(201_000)
    expect(clock.simulatedDays()).toBe(1599)
    for (const invalid of [NaN, Infinity, -Infinity]) clock.scrub(invalid)
    expect(clock.simulatedDays()).toBe(1599)
    expect(clock.playing()).toBe(false)
    clock.scrub(-1)
    expect(clock.simulatedDays()).toBe(0)
    clock.scrub(MAX_SIMULATED_DAYS + 1)
    expect(clock.simulatedDays()).toBe(MAX_SIMULATED_DAYS)
    clock.play()
    expect(frames.size).toBe(0)
  })

  it('resets playing time and speed with no remaining frame or stale resume interval', () => {
    const clock = start()
    clock.setSpeed(1000)
    tick(0)
    tick(1000)
